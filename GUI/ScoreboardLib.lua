...
function ScoreBoardManager:RenderServerInfo(xpos, ypos, r, g, b, xScale, yScale)
	-- Mixer: this code has been moved from here but stay this func intact to maintain compatibility!
	-- Mixer: added coop mission text here:
	if (Hud) and (Hud.coop_mission_text) then
		local namesizex,namesizy = %Game:GetHudStringSize(Hud.coop_mission_text, 20, 20);
		namesizex=400-(namesizex*0.5);
		%Game:WriteHudString(namesizex,ypos+6,Hud.coop_mission_text, 1, 1, 1, 1, 20,20);
	end
end
-- render team display window
