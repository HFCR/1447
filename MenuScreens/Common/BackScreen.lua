UI.PageBackScreen=
{
	GUI=
	{
		...
		Version=
		{
			...
			text = "$1v 1.7 $1© 2004 Crytek, $8Hunter_FC, $1Victor_Street",
		},
		Version2=
		{
			classname = "static",
			...
			halign = UIALIGN_RIGHT,
			mod_url = "$3http://vk.com/farcry_3top",
			text = "$1Far Cry: $8Return$1 $1Cry$3Top$1",
		},
...
Language:LoadStringTable("../Scripts/cismapsru.xml"); else
Language:LoadStringTable("../Scripts/cismapsen.xml"); end
setglobal("gr_MinTeamLimit",0);
setglobal("log_FileVerbosity",0);
setglobal("cl_lazy_weapon",0.3);
setglobal("e_vegetation_min_size",0);
-- Garcy, Garbitos, Mixer: important stuff to improve cnetwork response
-- setglobal("cl_cmdrate",100);
--------
...
UI:DisableWidget(Ad);
		...
		OnActivate = function(Sender)
			if (strfind(UI.PageBackScreen.GUI.Version2.text,System.this_mod_version) == nil) then
				UI.PageBackScreen.GUI.Version2:SetText(UI.PageBackScreen.GUI.Version2.text.." v"..System.this_mod_version);
			end
			UI:ShowMouseCursor();
			UI:PlayMusic();
			UI.PageBackScreen:PlayRandomVideo();
			...
			if (bkvideo==0) then
				UI:HideWidget(UI.PageBackScreen.GUI.Footer);
				...
				UI:ShowWidget(UI.PageBackScreen.GUI.Footer);
			end
