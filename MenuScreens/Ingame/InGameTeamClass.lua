Script:LoadScript("SCRIPTS/MULTIPLAYER/MultiplayerClassDefiniton.lua",1);
...
function UI.PageInGameTeamClass.GUI:OnActivate()
	...
	if (ipAddr) then
		UI.PageInGameTeamClass.GUI.IPName:SetText("$1IP: "..ipAddr.." ");
		ipAddr = tostring(getglobal("g_LastServerName"));
		UI.PageInGameTeamClass.GUI.ServerName:SetText(ipAddr);
	end
...
function UI.PageInGameTeamClass.GUI.OnClassChanged(Sender)
	...
	Sender:SetChecked(1);
	if (MultiplayerClassDefiniton.PlayerClasses.Support) then
	...
	for i=1, 4 do			
		...
		for WeaponIndex, WeaponName in ClassDef["weapon"..i] do
			if (WeaponName == "RL") and (toNumberOrZero(getglobal("gr_norl"))==1) then
			else
