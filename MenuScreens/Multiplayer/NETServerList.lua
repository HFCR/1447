UI.PageNETServerList=
{
	...
	{	
		...
		{			
			OnInit = function (Sender)
				...
				Sender:AddColumn("@IP", 97, UIALIGN_CENTER, UI.szListViewOddColor, "0 0 0 0");
				Sender:AddColumn("@GameType", 58, UIALIGN_CENTER, UI.szListViewEvenColor, "0 0 0 64");
		...
		TotalPlCount=
		{
			...
			width = 340,
			halign = UIALIGN_LEFT,
			text = "@cis_t_pl 0. @cis_f_pl 0, @cis_m_pl 0.",
		},
	},
	...
	PrepareToJoin = function()
		...
		if (iSelected) then
			...
			local checkver = strsub(UI.PageNETServerList.Servers[iSelected].Name, strlen(UI.PageNETServerList.Servers[iSelected].Name)-strlen(System.this_mod_version)+1);
			
			if (checkver) and (strlower(checkver) == strlower(System.this_mod_version)) then
			else
				UI.MessageBox("@Error", "@cis_mismod $3"..UI.PageBackScreen.GUI.Version2.mod_url.."$1 @cis_mismod2");
				return;
			end
	...
	AddServerToList = function(Server)
		...
		local szPingFilter = UI.PageMultiplayer.GUI.FilterBlock.PingCombo:GetSelection()
			...
			UI.CIS_totalplayers = UI.CIS_totalplayers + Server.Players;
			...
			if (strlower(szGameType) ~= strlower(Server.GameType) and UI.PageMultiplayer.GUI.FilterBlock.GameTypeCombo:GetSelectionIndex()~=1) then
				vetoAddToList = 1;
			end
			...
			if (tonumber( UI.PageMultiplayer.GUI.Mod_Filter:GetSelectionIndex() ) == 1) and (strupper(Game:GetCurrentModName())~=strupper(Server.Mod)) then
				vetoAddToList = 1;
			end
			...
			if(vetoAddToList == 0) then
				...
				UI.CIS_fltplayers = UI.CIS_fltplayers + Server.Players;
				if (strupper(Server.Mod) == strupper(Game:GetCurrentModName())) then
					UI.CIS_modplayers = UI.CIS_modplayers + Server.Players;
				end
				UI.PageNETServerList.GUI.TotalPlCount:SetText("@cis_t_pl "..UI.CIS_totalplayers..". @cis_f_pl "..UI.CIS_fltplayers..", @cis_m_pl "..UI.CIS_modplayers..".");
				-- add punkbuster icon			
