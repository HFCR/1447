UI.PageLANServerList=
{
	...
	PrepareToJoin = function()
		...
		if (iSelected) then
			...
			local checkver = strsub(szServerName, strlen(szServerName)-strlen(System.this_mod_version)+1);
		...
		for iIndex, Server in ServerList do		
			...
			if (tonumber( UI.PageMultiplayer.GUI.Mod_Filter:GetSelectionIndex() ) == 1) and (strupper(Game:GetCurrentModName())~=strupper(Server.Mod)) then
				...
				if(vetoAddToList == 0) then
					...
					UI.PageLANServerList.GUI.TotalPlCount:SetText("@cis_t_pl "..UI.CIS_totalplayers..". @cis_f_pl "..UI.CIS_fltplayers..", @cis_m_pl "..UI.CIS_modplayers..".");
