UI.PageServerAdmin=
{
        ...
        BanPlayer=
        {
            ...
            OnCommand = function(Sender)
                if (UI.PageServerAdmin.GUI.RCONPlayerList:GetText() ~= nil) then
					...
					local Reason = UI.PageServerAdmin.GUI.RCONReasonText:GetText();
					if not Reason then Reason = "@No_reason_given" end
					if (Game:IsServer()) then
						System:ExecuteCommand("banid "..PlayerToBan.." "..Reason);
					else
						Game:ExecuteRConCommand("banid "..PlayerToBan.." "..Reason);
					end
                    System:ShowConsole(1);
                end
            end,
	...
	WeapList=
		{
			...
			left = 317-144-144, top = (105+(24+6)*11)-30-30-30-30-30,
			width = 140, height = 24,
			tabstop = 264,
			OnChanged = function(Sender)
				UI.PageServerAdmin.GUI.WeapList.lastchosenname = UI.PageServerAdmin.GUI.WeapList:GetSelection().."";
			end,
		},
	------------
	VehicleList=
		{
			...
			left = 317-144-144, top = (105+(24+6)*11)-30,
			width = 140, height = 24,
			tabstop = 265,
			OnChanged = function(Sender)
				UI.PageServerAdmin.GUI.VehicleList.lastchosenname = UI.PageServerAdmin.GUI.VehicleList:GetSelection().."";
			end,
		},
	----
	Vehicle2ChosenPlayer=
	{
            skin = UI.skins.BottomMenuButton,
           ...
           left = 317-144-144, top = 105+(24+6)*11,
            width = 140, height = 24,

            OnCommand = function(Sender)
                if (UI.PageServerAdmin.GUI.RCONPlayerList:GetText() ~= nil) then
			local PlayerToKick = UI.PageServerAdmin.GUI.RCONPlayerList:GetText();
			local cgun = UI.PageServerAdmin.GUI.VehicleList:GetSelection();
			if (Game:IsServer()) then
				System:ExecuteCommand("gr_vehicle_to_player "..PlayerToKick.." "..cgun);
			else
				Game:ExecuteRConCommand("gr_vehicle_to_player "..PlayerToKick.." "..cgun);
			end
                end
            end,
        },
	...
	ItemList=
		{
			...
			left = 317-144-144, top = (105+(24+6)*11)-30-30-30,
			width = 140, height = 24,
			tabstop = 267,
			OnChanged = function(Sender)
				UI.PageServerAdmin.GUI.ItemList.lastchosenname = UI.PageServerAdmin.GUI.ItemList:GetSelection().."";
			end,
		},
	----
	Item2ChosenPlayer=
	{
           ...
           left = 317-144-144, top = (105+(24+6)*11)-30-30,
            width = 140, height = 24,

            OnCommand = function(Sender)
                if (UI.PageServerAdmin.GUI.RCONPlayerList:GetText() ~= nil) then
			local PlayerToKick = UI.PageServerAdmin.GUI.RCONPlayerList:GetText();
			local cgun = UI.PageServerAdmin.GUI.ItemList:GetSelection();
			if (Game:IsServer()) then
				System:ExecuteCommand("gr_item_to_player "..PlayerToKick.." "..cgun);
			else
				Game:ExecuteRConCommand("gr_item_to_player "..PlayerToKick.." "..cgun);
			end
                end
            end,
        },
	...
	--{
        --    skin = UI.skins.BottomMenuButton,
           ...
           --left = 317-144-144, top = (105+(24+6)*11)-30-30-30-30,
           -- width = 140, height = 24,
		...
		--	local cgun = UI.PageServerAdmin.GUI.ItemList:GetSelection();
		--	if (Game:IsServer()) then
		--		System:ExecuteCommand("gr_activate_ais "..PlayerToKick);
		--	else
		--		Game:ExecuteRConCommand("gr_activate_ais "..PlayerToKick);
		--	end
                --end
            --end,
        --},
	...
	EquipChosenPlayer=
	{
           ...
           width = 140, height = 24,
            ...
            OnCommand = function(Sender)
                if (UI.PageServerAdmin.GUI.RCONPlayerList:GetText() ~= nil) then
			local PlayerToKick = UI.PageServerAdmin.GUI.RCONPlayerList:GetText();
			local cgun = UI.PageServerAdmin.GUI.WeapList:GetSelection();
			if (Game:IsServer()) then
				System:ExecuteCommand("gr_equip_player "..PlayerToKick.." "..cgun);
			else
				Game:ExecuteRConCommand("gr_equip_player "..PlayerToKick.." "..cgun);
			end
                end
            end,
        },
        ...
        KickPlayer=
        {
            ...
            OnCommand = function(Sender)
                if (UI.PageServerAdmin.GUI.RCONPlayerList:GetText() ~= nil) then
					...
					if not Reason then Reason = "@No_reason_given" end
					if (Game:IsServer()) then
						System:ExecuteCommand("kickid "..PlayerToKick.." "..Reason);
					else
						Game:ExecuteRConCommand("kickid "..PlayerToKick.." "..Reason);
					end
                    System:ShowConsole(1);
                end
            end,
	...
	KillChosenPlayer=
	{
            ...
            left = 405+44+44+44, top = 105+(24+6)*11,
            width = 40, height = 24,
			...
			local PlayerToKick = UI.PageServerAdmin.GUI.RCONPlayerList:GetText();
			if (Game:IsServer()) then
				System:ExecuteCommand("gr_kill_player "..PlayerToKick);
			else
				Game:ExecuteRConCommand("gr_kill_player "..PlayerToKick);
			end
                end
            end,
        },
	HealChosenPlayer=
	{
			...
			local PlayerToKick = UI.PageServerAdmin.GUI.RCONPlayerList:GetText();
			if (Game:IsServer()) then
				System:ExecuteCommand("gr_heal_player "..PlayerToKick);
			else
				Game:ExecuteRConCommand("gr_heal_player "..PlayerToKick);
			end
                end
            end,
        },
	...
	SupplyChosenPlayer=
	{
			...
			local PlayerToKick = UI.PageServerAdmin.GUI.RCONPlayerList:GetText();
			if (Game:IsServer()) then
				System:ExecuteCommand("gr_supply_player "..PlayerToKick);
			else
				Game:ExecuteRConCommand("gr_supply_player "..PlayerToKick);
			end
                end
            end,
        },
	------
	  ...
	  RealReloadText=
	    ...
	    UI.PageServerAdmin.GUI.WeapList:Clear();
	     for name, MD in WeaponClassesEx do
		--if (name ~= "MG") then
			 UI.PageServerAdmin.GUI.WeapList:AddItem(name);
		--end
	     end
	     if (UI.PageServerAdmin.GUI.WeapList.lastchosenname) then
		UI.PageServerAdmin.GUI.WeapList:Select(UI.PageServerAdmin.GUI.WeapList.lastchosenname);
	     elseif (_localplayer) and (_localplayer.cnt) and (_localplayer.cnt.weapon) then
		UI.PageServerAdmin.GUI.WeapList:Select(_localplayer.cnt.weapon.name);
	     end

	      UI.PageServerAdmin.GUI.VehicleList:Clear();
	      for jj, name in EntityClassRegistry do
		if (name[1]) and (name[1]=="Vehicle") then
			if (name[2]~="BoatPatrol") then
				 UI.PageServerAdmin.GUI.VehicleList:AddItem(name[2]);
			end
		end
	      end
		if (UI.PageServerAdmin.GUI.VehicleList.lastchosenname) then
			UI.PageServerAdmin.GUI.VehicleList:Select(UI.PageServerAdmin.GUI.VehicleList.lastchosenname);
		else
			UI.PageServerAdmin.GUI.VehicleList:Select("Buggy");
		end

	     UI.PageServerAdmin.GUI.ItemList:Clear();
	     UI.PageServerAdmin.GUI.ItemList:AddItem("Armor");
	      UI.PageServerAdmin.GUI.ItemList:AddItem("Binoculars");
	       UI.PageServerAdmin.GUI.ItemList:AddItem("Flashlight");
	        UI.PageServerAdmin.GUI.ItemList:AddItem("Health");
	       UI.PageServerAdmin.GUI.ItemList:AddItem("HeatVisionGoggles");
	        UI.PageServerAdmin.GUI.ItemList:AddItem("Salo");
		 UI.PageServerAdmin.GUI.ItemList:AddItem("Vodka");
		 UI.PageServerAdmin.GUI.ItemList:AddItem("Yoghurt");

		if (UI.PageServerAdmin.GUI.ItemList.lastchosenname) then
			UI.PageServerAdmin.GUI.ItemList:Select(UI.PageServerAdmin.GUI.ItemList.lastchosenname);
		else
			UI.PageServerAdmin.GUI.ItemList:Select("Binoculars");
		end
