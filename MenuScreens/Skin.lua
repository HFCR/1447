function DefOnMouseEnter(self)
	...
	Sound:PlaySound(UI.EnterFrame_Sound);
end
...
function DefOnGotFocus(self)
	self:SetBorderColor(UI.szFocusColor);
	if (not UI.cis_fclik_snd) then
		UI.cis_fclik_snd=Sound:LoadSound( "Sounds/items/select.wav",SOUND_UNSCALABLE);
	end
	if (UI.cis_fclik_snd) then
		Sound:PlaySound(UI.cis_fclik_snd);
	end
	if (UI.MusicId) and (Sound:IsPlaying(UI.MusicId)==nil) then
		UI:PlayMusic();
	end
end
...
function DefOnMouseEnterSideButton(self)
	...
	Sound:PlaySound(UI.Hover_Sound);
end
