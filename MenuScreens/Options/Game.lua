...
UI.PageOptionsGame=
{
	GUI=
	{
		...
		hqsettings=
		{
			...
			OnChanged=function(Sender)
				if (Sender:GetChecked()) then
					if UI.PageOptionsVideoAdv.user then
						UI.PageOptionsVideoAdv.user:SetToVeryHighSpec()
						System:SaveConfiguration()
					end
					setglobal("r_RenderMode", 2 )
					setglobal("r_Contrast",0.6)
					setglobal("r_Brightness",0.45)
					setglobal("r_CoronaFade",0.13)
					setglobal("r_FSAA",1)
					setglobal("r_FSAA_quality",0)
					setglobal("r_FSAA_samples",8)
					System:Log("HQ Quality ON")
				else
					setglobal("r_FSAA",0)
...
--			UI.PageOptionsGame.GUI.lefthanded:SetChecked(g_LeftHanded);
			...
			if	tonumber( getglobal( "r_RenderMode" ) ) == 2 and
				tonumber( getglobal( "r_Contrast" ) ) == 0.57 and
				tonumber( getglobal( "r_Brightness" ) ) == 0.5 and
				tonumber( getglobal( "r_CoronaFade" ) ) == 0.12 and
				tonumber( getglobal( "r_FSAA" ) ) == 1  and
				tonumber( getglobal( "r_FSAA_quality" ) )  == 0 and
				tonumber( getglobal( "r_FSAA_samples" ) ) == 8  then
				UI.PageOptionsGame.GUI.hqsettings:SetChecked(1)
			else
				UI.PageOptionsGame.GUI.hqsettings:SetChecked(0)
			end
