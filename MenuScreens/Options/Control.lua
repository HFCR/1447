...
function UI.PageOptionsControl.FillBindList()
	...
	for szType, Map in ActionMap do
		...
		for i, szActionName in MapOrder do
			--System:LogAlways(szActionName);
			for j, Action in Map do
						
				-- find the key in the list of keys with this szType
				if (strlower(strsub(Action.desc, 2)) == strlower(szActionName)) or (strlower(strsub(Action.desc, 1)) == strlower(szActionName)) then
				...
				-------------------------
				elseif (szActionName=="KnifeMash") then
					local sKnifeKey = UI:Ecfg("bin32/rc.ini","KnifeMash");
					if (not sKnifeKey) then
						sKnifeKey = "capslock";
						UI:Ecfg("bin32/rc.ini","KnifeMash",sKnifeKey);
					end
					local iIndex = UI.PageOptionsControl.GUI.controllist:AddItem("@"..szActionName, sKnifeKey, "");
					if (iIndex) then
						UI.PageOptionsControl.Actions[iIndex] = {
							configurable = 1,
							actionmaps = {1},
							weird_id = szActionName,
							desc = "@"..szActionName,
						};
						break;
					end
				------------------------------
				elseif (szActionName=="SpecialOption") then
					local sSpOptKey = UI:Ecfg("bin32/rc.ini","SpOptionKey");
					if (not sSpOptKey) then
						sSpOptKey = "5";
						UI:Ecfg("bin32/rc.ini","SpOptionKey",sSpOptKey);
					end
					local iIndex = UI.PageOptionsControl.GUI.controllist:AddItem("@"..szActionName, sSpOptKey, "");
					if (iIndex) then
						UI.PageOptionsControl.Actions[iIndex] = {
							configurable = 1,
							actionmaps = {1},
