...
-----
Game:AddCommand("gr_kill_player","SVcommands:Kill_Player(%1);","gr_kill_player [player_id]"); --players (except protected) cannot join a team
Game:AddCommand("gr_heal_player","SVcommands:Heal_Player(%1);","gr_heal_player [player_id]"); --players (except protected) cannot join a team
Game:AddCommand("gr_supply_player","SVcommands:Supply_Player(%1);","gr_supply_player [player_id]"); --players (except protected) cannot join a team
Game:AddCommand("gr_equip_player","SVcommands:Equip_Player(%1,%2);","gr_equip_player [player_id weapon_name]"); --players (except protected) cannot join a team
Game:AddCommand("gr_vehicle_to_player","SVcommands:Vehicle2_Player(%1,%2);","gr_vehicle_to_player [player_id vehicle_name]"); --players (except protected) cannot join a team
Game:AddCommand("gr_item_to_player","SVcommands:Item2_Player(%1,%2);","gr_item_to_player [player_id item_name]"); --players (except protected) cannot join a team
--Game:AddCommand("gr_activate_ais","SVcommands:Activate_Ais(%1);","gr_activate_ais [player_id]"); --players (except protected) cannot join a team
--
function SVcommands:Equip_Player(pl_id,wp_name)
	local KickSlot = Server:GetServerSlotBySSId(pl_id);
	if (KickSlot) then
		local plid = KickSlot:GetPlayerId();
		if (plid) then
			plid = System:GetEntity(plid);
			if (plid) and (plid.cnt) and (plid.cnt.health > 0) then
				------
				--BasicPlayer.ScriptInitWeapon(plid, wp_name,1);
				plid.cnt:MakeWeaponAvailable(WeaponClassesEx[wp_name].id);
				plid.cnt:SetCurrWeapon(WeaponClassesEx[wp_name].id);
				if (plid.fireparams) and (plid.fireparams.bullets_per_clip) then
					plid.cnt.ammo_in_clip = plid.fireparams.bullets_per_clip * 1;
					if (plid.cnt.ammo) and (plid.cnt.ammo < plid.cnt.ammo_in_clip*2) then
						plid.cnt.ammo = plid.fireparams.bullets_per_clip * 2;
					end
				end
				-------
			end
...
--function SVcommands:Activate_Ais(pl_id)
--	local sais = System:GetEntities();
--	for j,sai in sais do
--		if (sai.isbadmonster) then
--			if (sai.cnt) and (sai.cnt.health > 0) then
--				sai:EnableUpdate(1);
--				sai:TriggerEvent(AIEVENT_WAKEUP);
--			end
--		end
--	end
--end

function SVcommands:Item2_Player(pl_id,wp_name)
	local KickSlot = Server:GetServerSlotBySSId(pl_id);
	if (KickSlot) then
		local plid = KickSlot:GetPlayerId();
		if (plid) then
			...
			if (plid) and (plid.cnt) and (plid.cnt.health > 0) then
				------
				--BasicPlayer.ScriptInitWeapon(plid, wp_name,1);
				local plangle = new(plid:GetAngles());
				plid:SetAngles({x=0,y=0,z=plangle.z});
				local plpos = new(plid:GetPos());
				local pldir = new(plid:GetDirectionVector());
				pldir.x = plpos.x + pldir.x * 1.5;
				pldir.y = plpos.y + pldir.y * 1.5;
				pldir.z = plpos.z + pldir.z * 1.5;
				pldir.z=pldir.z+1.7;
				plangle.z=plangle.z+180;
				
				local cid = Game:GetEntityClassIDByClassName(wp_name);
				if (cid==nil) then
					cid = Game:GetEntityClassIDByClassName("Pickup"..wp_name);
				end
				if (cid) then
					local vhcl1 = Server:SpawnEntity({classid=cid,pos=pldir,angles=plangle,name=plid:GetName().."'s "..wp_name});
					if (vhcl1) then
						if (vhcl1.Properties.RespawnTime) and (vhcl1.SetFadeTime) then
							vhcl1.Properties.RespawnTime = 30;
							vhcl1.vOrigPos = pldir;
						elseif (vhcl1.cnt) and (vhcl1.cnt.health) then
							vhcl1.deleteOnGameReset = 1;
							vhcl1.bot_born = 1;
							vhcl1.isbadmonster = vhcl1.id*1;
							vhcl1.assault_score = 0;
							vhcl1.bot_killstreak = 0;
					...
					System:LogAlways("Cant find "..wp_name);
				end
				-------
			end
		end
...
function SVcommands:Vehicle2_Player(pl_id,wp_name)
	local KickSlot = Server:GetServerSlotBySSId(pl_id);
	if (KickSlot) then
		local plid = KickSlot:GetPlayerId();
		if (plid) then
			...
			if (plid) and (plid.cnt) and (plid.cnt.health > 0) then
				...
				local plpos = new(plid:GetPos());
				local pldir = new(plid:GetDirectionVector());
				pldir.x = plpos.x + pldir.x * 8;
				pldir.y = plpos.y + pldir.y * 8;
				pldir.z = plpos.z + pldir.z * 8;
				pldir.z=pldir.z+1.7;
				plangle.z=plangle.z+180;
				
				local cid =Game:GetEntityClassIDByClassName(wp_name);
				if (cid) then
					local vhcl1 = Server:SpawnEntity({classid=cid,pos=pldir,angles=plangle});
					if (vhcl1) then
						vhcl1.PhoenixData = {
							pos = pldir,
							angles=plangle,
						};
						if wp_name == "Paraglider" then
							vhcl1.Properties.fAbandonedTime = 10;
						end
						vhcl1.admin_spawned = 1;
					end
				end
				-------
			end
...
function SVcommands:Kill_Player(pl_id)
	local KickSlot = Server:GetServerSlotBySSId(pl_id);
	if (KickSlot) then
		local plid = KickSlot:GetPlayerId();
		if (plid) then
			plid = System:GetEntity(plid);
			if (plid) and (plid.cnt) and (plid.cnt.health > 0) then
				GameRules:OnDamage({ target = plid, shooter = plid, damage = 99999, ipart = 0 });
			end
		end
	end
end

function SVcommands:Heal_Player(pl_id)
	local KickSlot = Server:GetServerSlotBySSId(pl_id);
	if (KickSlot) then
		local plid = KickSlot:GetPlayerId();
		if (plid) then
			...
			if (plid) and (plid.cnt) and (plid.cnt.health > 0) then
				--GameRules:OnDamage({ target = plid, shooter = plid, damage = 99999, ipart = 0 });
				plid.cnt.armor = 100;
				plid.cnt.health = plid.cnt.max_health*1;
			end
		end
	end
end

function SVcommands:Supply_Player(pl_id)
	local KickSlot = Server:GetServerSlotBySSId(pl_id);
	if (KickSlot) then
		local plid = KickSlot:GetPlayerId();
		if (plid) then
			plid = System:GetEntity(plid);
			if (plid) and (plid.cnt) and (plid.cnt.health > 0) and (plid.cnt.weapon) and (plid.cnt.ammo_in_clip) then
				--GameRules:OnDamage({ target = plid, shooter = plid, damage = 99999, ipart = 0 });
				plid.cnt.ammo_in_clip = 999;
			end
		end
	end
end
