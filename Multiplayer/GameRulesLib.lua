...
GameRules.ClientCommandTable["HSS"]=function(String,server_slot,toktable) -- client to server hit stuff
	local hs = 3;
	local ht = {};
	ht.shooter = System:GetEntity(server_slot:GetPlayerId());
	if ht.shooter.cnt and ht.shooter.fireparams then
		ht.dir = ht.shooter:GetDirectionVector();
		ht.impact_force_mul = ht.shooter.fireparams.iImpactForceMul;
		ht.impact_force_mul_final = ht.shooter.fireparams.iImpactForceMulFinal;
		ht.weapon = ht.shooter.cnt.weapon;
		
		----- loki check add here
		ht.ac_loki = 0;
		for i,j in ht.shooter.fireparams do
			if type(j)=="number" then
				ht.ac_loki = ht.ac_loki + j;
			end
		end
		ht.landed = tonumber(toktable[2]);
		ht.ac_loki = ht.ac_loki + tonumber(getglobal(g_Vectors.temp_v5..g_Vectors.v112));
		
		if ht.landed and ht.landed == floor(ht.ac_loki*1000) then
			
			while toktable[hs] do

				ht.landed = strfind(toktable[hs],"a");
				ht.target = strsub(toktable[hs],1,ht.landed-1);
				ht.target = System:GetEntity(ht.target);
				ht.clpredictedhit = strfind(toktable[hs],"/");
				ht.target_material = "mat_"..strsub(toktable[hs],ht.landed+1,ht.clpredictedhit-1);
				
				ht.target_material = Game:GetMaterialBySurfaceID(Game:GetMaterialIDByName(ht.target_material));
				ht.ipart = strsub(toktable[hs],ht.clpredictedhit+1);
				ht.ipart = tonumber(ht.ipart);
				if ht.ipart and ht.ipart == -1 then
					ht.ipart = nil;
				end

				ht.pos = ht.target:GetPos();
				if ht.shooter.fireparams.damage_drop_per_meter then
					ht.dist = ht.shooter:GetDistanceFromPoint(ht.pos);
					ht.damage = ht.shooter.fireparams.damage - ht.shooter.fireparams.damage_drop_per_meter * ht.dist;
				else
					ht.damage = ht.shooter.fireparams.damage * 1;
				end

				ht.weapon.Server.OnHit(ht.weapon,ht);
				ht.target:Damage( ht ); -- dmg

				hs = hs + 1;
			end
		else
			server_slot:SendText("Stop cheating, you can't harm anyone!");
		end
	end
end
...
--	end
	...
	if (GameRules.ModeDesc) then
		if GameRules:ModeDesc() == "HUNT" then
			if team == "blue" then
				_model = "Objects/characters/mutants/mutant_stealth/mutant_stealth.cgf";
				server_slot.is_mutatd_slot = 1;
			end
		end
	end
...
--	server_slot:SendCommand("GI WS");		-- Only do this once in OnAfterSpawnEntity
	...
	if(team=="spectators") then				
		...
		if (server_slot.MyHouseId) then
			local mhe=System:GetEntity(server_slot.MyHouseId);
			if mhe then 
			
				if mhe.Properties.nAmount and mhe.Properties.nAmount > 1 then -- multi-flat house
					rp = random(1,mhe.Properties.nAmount);
					rp=Game:GetTagPoint(mhe:GetName().."_"..rp);
				else   -- single-flat house
					rp=Game:GetTagPoint(mhe:GetName());
				end
				System:Log("rp: " .. tostring(rp));
			end
		else
...
--	dump(team_color);
	...
	if (server_slot.is_mutatd_slot) then
		newent.is_mutatd_sv = 1;
	end
	server_slot.is_mutatd_slot = nil;
...
function GameRules:HandleJoinTeamRequest(server_slot,new_team)
	...
	if (not server_slot.firstjoined) then
		server_slot:SendCommand("PMUS "..getglobal("gr_server_music"));
		server_slot.firstjoined = 1;
	end
...
-------------------------------------------------------------------------------
-- Kicks a player
function GameRules:Kick(player,Reason)
...
--	System:Log("GameRules:Kick "..player.." "..tostring(KickSlot));
	...
	GameRules:KickSlot(KickSlot,Reason);	
end

-------------------------------------------------------------------------------
--
function GameRules:KickID(id,Reason)
...
-------------------------------------------------------------------------------
--
function GameRules:KickSlot(ServerSlot,Reason)
...
-- --		printf("kicked player id %d...", KickIDServerSlot:GetPlayerId());
	...
	System:Log("kicked player "..tostring(ServerSlot:GetName()).."$1. Reason: "..Reason);
	Server:BroadcastText("@KickingPlayer "..tostring(ServerSlot:GetName()).."$1. @Reason2 "..Reason);
	ServerSlot:Disconnect("@Kicked @Reason "..Reason);
end
...
-------------------------------------------------------------------------------
--
function GameRules:BanID(id,Reason)
	if (not id or strlen(id) < 1) then
		return;
	end
	...
	GameRules:BanSlot(BanSlot,Reason);
end

-------------------------------------------------------------------------------
--
function GameRules:BanSlot(ServerSlot,Reason)
	if (_localplayer and ServerSlot) then
		if (_localplayer.id == ServerSlot:GetPlayerId()) then
	...
	if (ServerSlot) then
		ServerSlot:BanByID();
		GameRules:KickSlot(ServerSlot,Reason);
	end
end
