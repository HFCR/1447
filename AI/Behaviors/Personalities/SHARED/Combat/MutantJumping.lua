...
AIBehaviour.MutantJumping = {
	...
	OnCloseContact = function ( self, entity, sender)
		if entity.sim_melee_hit == nil then
			if entity.MELEE_ANIM_COUNT and entity.MELEE_ANIM_COUNT > 0 and entity.cnt and entity.cnt.weapon==nil then
				local evel  = entity:GetVelocity();
				if entity.lastMeleeAttackTime < _time and evel.z < 0 and entity.cnt.flying then
					entity.lastMeleeAttackTime = _time + random(1,2);
					entity.sim_melee_hit = 0;
				end
			end
		end
	end,
		...
		if (self.fLastJumpDuration>0) then
			...
			if (JMPTable) then
				...
				if (Game:IsMultiplayer()) then
					...
					Server:BroadcastCommand("PLAA "..entity.id.." "..JMPTable[1]);
				end
			end	
