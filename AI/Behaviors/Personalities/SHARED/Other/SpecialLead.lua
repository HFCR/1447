...
AIBehaviour.SpecialLead = {
	...
	OnCloseContact = function( self, entity, sender)
		if (entity.LEADING==nil) then
			...
			local tgp2 = Game:GetTagPoint(tgPoint);
			if (tgp2) then
				entity.LEADING = 1;
				if (entity:GetDistanceFromPoint(tgp2)>1) then
	...
	YOU_CAN_APPROACH_TO_NEXT = function (self, entity, sender)
		...
		-- KoTimon
		-- if entity.Properties.species~=_localplayer.Properties.species then -- Если юнит не принадлежит к стороне игрока, то разрешить ему стрелять.
			AI:CreateGoalPipe("standingthere_lead");
			AI:PushGoal("standingthere_lead","bodypos",0,0);
			AI:PushGoal("standingthere_lead","firecmd",0,1);
		-- end
	
		-- entity:SelectPipe(0,"standingthere");
		entity:SelectPipe(0,"standingthere_lead");
