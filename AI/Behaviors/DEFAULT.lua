...
AIBehaviour.DEFAULT = {
	...
	RUSH_TARGET = function ( self, entity, sender)
		...
		if (entity.items) and (entity.items.pissed_by_player) then
			entity:Readibility("THREATEN_GROUP",1);
		else
	...
	START_SWIMMING = function ( self, entity, sender)
		-- Mixer: these 2 next lines are commented out due to stupid behavior of AIs on getting in water, even knee-deep
		--AI:MakePuppetIgnorant(entity.id,1);
...
--		end
	...
	DO_THREATENED_ANIMATION = function(self,entity,sender)
		...
		if (Game:IsMultiplayer()) then
			-- can be seen in client of survival dedicated server
			Server:BroadcastCommand("PLAA "..entity.id.." _surprise0"..rnd);
		end
