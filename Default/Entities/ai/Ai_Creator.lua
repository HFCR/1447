...
function Ai_Creator:OnPropertyChange()
	self.pickZombie = nil;
	self:OnReset();
end

function Ai_Creator:OnInit()
	self:OnReset();
end

function Ai_Creator:OnReset()
	self.spawnnum = 0;
...
function Ai_Creator:OnTimer()
	self.bWaiting = 0;
	self:Event_CreateAI();
end

function Ai_Creator:Event_EveryoneDestroyed()
	self.deathrow = 0;
	BroadcastEvent( self,"EveryoneDestroyed" );
end

function Ai_Creator:Event_KillSpawnedAis()
	local ents = System:GetEntities();
	for i,ent in ents do
		if (ent.isbadmonster) and (ent.isbadmonster == self.id) and (ent.Event_Die) then
			ent:Event_Die();
		end
	end
end

function Ai_Creator:Event_Enable()
	self.bEnabled = 1;
end

function Ai_Creator:Event_Disable()
	self.bEnabled = 0;
end

function Ai_Creator:Event_ResetState()
	self:OnReset();
	BroadcastEvent( self,"ResetState" );
end

function Ai_Creator:Event_ChargeAI()
...
function Ai_Creator:Event_Extinct()
	BroadcastEvent( self,"Extinct" );
end

function Ai_Creator:OnSave(stm)
...
function Ai_Creator:OnLoad(stm)
...
function Ai_Creator:Event_CreateAI()
	if (Game:IsServer()) and (self.bEnabled==1) then
		...
		if (self.Properties.fileCoopModel) and (self.Properties.fileCoopModel~="") then
			-- pick zombie model randomly
			if (self.Properties.fileCoopModel=="Zombie") or (self.pickZombie) then
				self.pickZombie = {
					"Objects/characters/mercenaries/Zombie/Zombie.cgf",
					"Objects/characters/mercenaries/Zombie/zombie_worker.cgf",
					"Objects/characters/mercenaries/Zombie/zombie_rear.cgf",
					"Objects/characters/mercenaries/Zombie/zombie_cover.cgf",
					"Objects/characters/mercenaries/Zombie/zombie_scout.cgf",
					"Objects/characters/mercenaries/Zombie/zombie_lab.cgf",
					"Objects/characters/mercenaries/Zombie/zombie_lead.cgf",
					"Objects/characters/mercenaries/Zombie/zombie_indoor_light.cgf",
					"Objects/characters/mercenaries/Zombie/zombie_indoor.cgf",
					"Objects/characters/mercenaries/Zombie/zombie_indoor_elite.cgf",
					"Objects/characters/mercenaries/Zombie/zombie_mertz.cgf",
				};
				self.Properties.fileCoopModel=self.pickZombie[random(1,11)].."";
			end
