--- MUTANTMONKEY (TWEAKED BY MIXER 2007) ---
MutantMonkey_x = {
...
function MutantMonkey_x:OnInitCustom()
	...
	if (self.Properties.ReinforcePoint == "MUTANT_AIRDUCT") then
		-- Mixer: set prone dimensions for all stances for airduct crawling mutants
		self.PlayerDimNormal = self.PlayerDimProne;
		self.PlayerDimCrouch = self.PlayerDimProne;
	end
	if (self.Properties.fMeleeDistance == 3) then
		-- monkey attack range fix for non-cis
		if (self.Properties.JUMP_TABLE == "BIG_ABBERATION") then
			self.Properties.fMeleeDistance = 1.9;
		else
			self.Properties.fMeleeDistance = 1.5;
		end
	end
end
