...
function BasicEntity:SetFromProperties()
	...
	if (self.bCharacter == 1) then
		...
		if (bMakeHandheld == 1) then
			if (self.Properties.Physics.bRigidBody~=0) and (self.Properties.sndAnimStop~="") then
				self:LoadObject( self.Properties.sndAnimStop,1,1 );
				self.cis_handheldobj = 1;
				self:DrawObject(1,0);
			else
				if (self.cis_handheldobj) then
					self:DrawObject(1,0);
				end
				self.cis_handheldobj = nil;
			end
		end
	end
...
function BasicEntity:SetPhysicsProperties( bPhysicalize,bActivateRigidBody )
  local Mass,Density,qual;
	...
	if (self:IsARigidBody() == 1) then
		...
		if (not ref_use_simple_solver) then
			Flags = { flags_mask=pef_fixed_damping, flags=0 };
		else
			Flags = { flags_mask=pef_fixed_damping+ref_use_simple_solver, flags=0 };
		end

		Flags.flags = self.Properties.Physics.bFixedDamping*pef_fixed_damping;
		if (self.Properties.Physics.bUseSimpleSolver) and (ref_use_simple_solver) then
...
function BasicEntity:OnReset()
	...
	if (self.last_hold_time) then
		self:EnablePhysics(1);
		self.last_hold_time = nil;
	end
	if (self:IsARigidBody()==1) then
			...
			if (self.LastPhysicsSounds and self.LastPhysicsSounds.ImpactVolume) then
				if (hit.impact) then
					...
					if (self.LastPhysicsSounds.Impact and finalvol>1) then
						...
						if (self.cis_lastgrabberid) then
							AI:SoundEvent(self.id,self:GetPos(),10,0,Volume,self.cis_lastgrabberid);
						end
					end
				end
				local Volume;
...
function BasicEntity:OnRemoteEffect(toktable, pos, normal, userbyte)
	if (not Game:IsServer()) then
		if (userbyte == 1) then -- client play
			self:Event_StartAnimation();
		elseif (userbyte == 0) then -- client stop
			self:Event_StopAnimtion();
		elseif (userbyte == 2) then -- client updatepos
			self:SetPos(pos);
