...
--Hud.label = "inside ProximityDamage entity";
	...
	if (self.Properties.bShakeOnly==0) then	
		...
		elseif (self.Properties.nDamageType==2) and (_localplayer) and (collider==_localplayer) then
			BasicPlayer.PlayInteractSound(_localplayer,"Sounds/mutants/Mslash3.wav");
			local ppos = _localplayer:GetPos();
			ppos.z = ppos.z + 0.3;
			Particle:SpawnEffect(ppos,{x=0,y=0,z=1},"blood.on_water.a");
		end
