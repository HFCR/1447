...
projectileDefinitionSP = {
	...
	ExplosionParams = {
		pos = {},
		damage = 55,
		rmin = 0.8,  --Defines the minimum impulse pressure range
		rmax = 0.8,  --Defines the maximum impulse pressure range
		radius = 0.8, --Damage radius where point 0 is max damage. Damage falls off at 1/distance^2
		DeafnessRadius = 0,
		DeafnessTime = 0,
	...
	if (status) then
		if (status.target) and (status.target.Damage) then
			...
			if (status.target.cnt) and (status.target.type == "Player") then
				hit.tempstt = ceil(status.target.cnt.max_health*0.5);
