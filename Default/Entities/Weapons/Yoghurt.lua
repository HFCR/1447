Yoghurt = {
	name			= "Yoghurt",
	object		= "objects/weapons/salo/yoghurt_pickup.cgf",
	...
	special_bone_to_bind = "Dummy12", --usually the weapon model is attached to "weapon_bone" bone, 
		...
		-- describes all supported firemodes
	{
		...
		FireSounds = {
			"objects/weapons/salo/yoghurt_drink.wav",
			},
		no_ammo=1,
...
function Yoghurt.Server:OnFire( params )
	if (params.shooter) then
		params.shooter.eatfoodtime = _time + params.shooter.fireparams.fire_rate * 0.8;
	end
	return 1;
end

function Yoghurt.Server:OnActivate(Params)
	if Params.shooter then
...
function Yoghurt.Server:OnUpdate(delta, shooter)	
	...
	if shooter.eatfoodtime and shooter.eatfoodtime < _time and shooter.cnt.ammo_in_clip > 0 then
		shooter.eatfoodtime = nil;
		if shooter.items.bodun_range then
			shooter.items.bodun_range = shooter.items.bodun_range * 0.6;
		else
			local health_up = shooter.cnt.max_health*0.15;
