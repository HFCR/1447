ShotgunDBN = {
	name		= "ShotgunDBN",
	object	= "Objects/Weapons/Shotgun_db_newz/shotgun_new_hfcr_bind.cgf",
	character	= "Objects/Weapons/Shotgun_db_newz/Shotgun_db.cgf",
	
	PlayerSlowDown = 1.1,									-- factor to slow down the player when he holds that weapon
	...
	FireParams ={
	{
		...
		FireSounds = {
			"Sounds/Weapons/COFS_Rifle/Rifle_Fire2.wav",
		},
		FireSoundsStereo = {
			"Sounds/Weapons/COFS_Rifle/Rifle_Fire2.wav",
		},
...
CreateBasicWeapon(ShotgunDBN);

ShotgunDBN.anim_table={}
--AUTOMATIC FIRE
ShotgunDBN.anim_table[1]={
	idle={
		"Idle11",
		"Idle21",
	},
...
function ShotgunDBN.Client:OnActivate(Params)
	BasicWeapon.Client.OnActivate(self,Params);
	if (Params.shooter == _localplayer) then
		self:LoadObject("Objects/Weapons/Shotgun_db_newz/shotgun_hfcr.cgf",2,1);
		self.cap_stuff1 = self:AttachObjectToBone( 2,"spitfire",1 );
	end
end

function ShotgunDBN.Server:OnFire( params )
	if (AIWeaponProperties) then
		if (not AIWeaponProperties.ShotgunDBN) then
			AIWeaponProperties.ShotgunDBN = {
				VolumeRadius = 200,
				fThreat = 1,
				fInterest = 0.5,
			};
...
function ShotgunDBN.Client:OnFire( Params )
...
function ShotgunDBN.Client:OnUpdate(delta,shooter)
