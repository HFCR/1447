...
local ExplodingEffect=function(self)
	if (self.stopSmokeTime) and (self.GetPos) then
		if (self.counter>0.6) and (self.stopSmokeTime > _time) then
			Particle:CreateParticle( self:GetPos(), {0,0,1}, self.Gas );
			self.counter=0;
		elseif (self.stopSmokeTime < _time-0.5) then
			-- Mixer: fix: stop smoke sound for dedicated server's client
			if (self.exploding) and (Sound:IsPlaying(self.exploding)) then
