-- Mixer: modified a bit for vbot compatibility 2.0 with built-in indoor fx
BaseHandMga={
	type="Projectile",
...
---------------------------------
	...
	model="objects/weapons/x850/x850_grenade.cgf",
	alienmodel="Objects/Pickups/Grenades/plasm_ball.cgf",
	model_scale = 1.0,
	softcoverscale=10,
};


function BaseHandMga:InitStates()
...
function BaseHandMga:InitGeometry()
...
function BaseHandMga:Server_Flying_OnUpdate(dt)
...
function BaseHandMga:Client_Flying_OnUpdate(dt)
	...
	if (not self.was_underwater) then
		...
		if (self.aworld) then
			self:AddDynamicLight(pos, 5, 1, 1, 0.3, 0.7, 0.7, 0.7, 1, 1, 0);
		end
	end
end

function BaseHandMga:Client_Exploding_OnUpdate(dt)
...
function BaseHandMga:Client_OnShutDown(dt)
...
function BaseHandMga:Launch( weapon, shooter, pos, angles, dir ,lifetime)
	...
	if (shooter.bot_classnum) and (shooter.bot_classnum == 0) then
	self.PhysParams.heading = shooter:GetDirectionVector(); else
	...
	elseif (shooter.Properties.KEYFRAME_TABLE == "MUTANT_STEALTH") and (shooter.cnt.weapon) then
		shooter.cnt.weapon_busy = 2.8;
	end
end

function BaseHandMga:OnSave(stm)
...
function BaseHandMga:OnLoad(stm)
...
function BaseHandMga:OnLoadRELEASE(stm)
end

----------------------------------
BaseHandMga.Server = {
	OnInit = BaseHandMga.Server_OnInit,
	Flying = {
		OnBeginState = function(self)
			--ensure that the grenade will explode also if doesn't collide(after sometimes)
			self:SetTimer(self.lifetime);
		end,
		OnUpdate = BaseHandMga.Server_Flying_OnUpdate,
...
BaseHandMga.Client = {
	OnInit = BaseHandMga.Client_OnInit,
	Flying = {
		OnUpdate = BaseHandMga.Client_Flying_OnUpdate,
	},
	Exploding = {
		OnBeginState = function(self)
			...
			if(not self.no_explosion)then
				...
				if((Game:GetWaterHeight(temppos)-temppos.z)>0)then
					...
					if (self.deform_terrain) and (not self.terrain_deformed) then
						self.terrain_deformed=1;
						if (System:IsPointIndoors(temppos)) then
							material = Materials.mat_default;
							if (material) and (not material.grenade_explosion_indoor) then
								material.grenade_explosion_indoor = {
									particleEffects = {
										name = "explosions.Grenade_indoors.explosion",
										scale = 1.5,
									},
								};
							end
							effect = "grenade_explosion_indoor";
						elseif (self.last_bounce == 0) or (abs(temppos.z - System:GetTerrainElevation(temppos))>0.95) then
		...
		OnUpdate = BaseHandMga.Client_Exploding_OnUpdate,
	...
	OnShutDown = BaseHandMga.Client_OnShutDown,
}
