...
AG36SP = {
	...
	character	= "Objects/Weapons/ag36_mod/ag36.cgf",
	...
	---------------------------------------------------
	ActivateSound = Sound:LoadSound("Sounds/Weapons/AG36/ag36weapact.wav",0,150),	-- sound to play when this weapon is selected
	---------------------------------------------------
	Sway=2,
	---------------------------------------------------
	DoesFTBSniping = 1,
	DrawFlare=1,
	wrong_material_assignment = 1,
	---------------------------------------------------

	FireParams ={													-- describes all supported 	firemodes
	{
		silenced="Sounds/Weapons/Ag36/AG36_Silent.wav",--		
		HasCrosshair=1,
		AmmoType="Assault",
		reload_time= 2.6,
		fire_rate= 0.1,
		...
		FireSounds = {
			"Sounds/weapons/ag36/g36single.wav",
			"Sounds/weapons/ag36/g36single.wav",
		},
		FireSoundsStereo = {
			"Sounds/weapons/ag36/g36single.wav",
...
function AG36.Client:OnEnhanceHUD(scale, bHit)
	...
	System:DrawImageColorCoords( AG36.ZoomBackgroundTID, 400, 600, 400+Hud.hdadjust*1.52, -600, 4, 1, 1, 1, 1, fTexelW, fTexelH, 1-fTexelW, 1-fTexelH);
	System:DrawImageColorCoords( AG36.ZoomBackgroundTID, 400, 600, -400-Hud.hdadjust*1.52, -600, 4, 1, 1, 1, 1, fTexelW, fTexelH, 1-fTexelW, 1-fTexelH);
end

function AG36:ScopeToScreen(zoomdiff)
	if (zoomdiff > 1) then
		zoomdiff = 1;
	end
	self.cnt:SetFirstPersonWeaponPos({x=0.17*zoomdiff,y=0.28*zoomdiff,z=0.05*zoomdiff},g_Vectors.v000);
end

if (Game:IsMultiplayer()) then
	function AG36.Server:OnUpdate(delta, shooter)
		if (shooter.fireparams.projectile_class) then
			local maxclip = tonumber(getglobal("gr_clip_ag36_gl"));
			if (maxclip) then
