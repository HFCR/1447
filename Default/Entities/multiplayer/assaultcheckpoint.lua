-- ASSAULTCHECKPOINT WITH HACK FOR VOMIT BOTS
-- MODIFIED BY MIXER 2010 v1.X3cis
ASSAULTCheckPoint={
Properties={
...
function ASSAULTCheckPoint:Event_Capturing()
	if self.Properties.bVisible==1 then
		...
		if (ss) then
			...
			local vbt = System:GetEntity(self.captureCollider);
			if (vbt) then
				if (toNumberOrZero(getglobal("gr_flag_startcapture_message"))==1) then
					Server:BroadcastText("$4>$2>$4> "..vbt:GetName().."$6 has activated the flag $4<$2<$4<");
				end
				if (vbt.POTSHOTS) and (vbt.cap_score) then -- this means the collider is bot for sure
					vbt.cap_score = vbt.cap_score + GameRules.ScoreMultipliers.xCaptureStarted;
					vbt:SetBotStats(1);
				end
			end
		end
...
-- VOMIT BOTS HACK ---------------------
if (collider.bot_born) then
--Hud:AddMessage("BOT ATTACKER TOUCH!",30);
self.captureCollider = collider.id; else
--Hud:AddMessage("REAL ATTACKER TOUCH!",30);
self.captureCollider = Slot:GetId(); end
-- VOMIT BOTS HACK ---------------------
...
OnTimer = function(self)
			...
			elseif (self.captureCollider) then
				...
				for i, ent in allents do
					if (ent.POTSHOTS) and (GameRules:IsDefender(ent)) then
						if (GameRules.vb_ass_go==nil) then
							GameRules.vb_ass_go = 1;
							ent.cnt.speedscale = 1.7;
							ent.bot_engaging = nil;
							ent:InsertSubpipe(0,"run_to_trigger",self.id);
						else
			...
			if (GameRules:IsDefender(collider) and (self:IsInRange(collider) == 1)) then
				...
				if (touch==1) then
					...
					if (ss) then
						...
						local vbt = System:GetEntity(collider.id);
						if (vbt) then
							if (toNumberOrZero(getglobal("gr_flag_saved_message"))==1) then
								Server:BroadcastText("$4>$2>$4> "..vbt:GetName().."$6 saved the flag $4<$2<$4<");
							end
							if (vbt.POTSHOTS) and (vbt.cap_score) then -- this means the collider is bot for sure
								vbt.cap_score = vbt.cap_score + GameRules.ScoreMultipliers.xCaptureAverted;
								vbt:SetBotStats(1);
							end
						end
					end
