...
--	ammoRL = 30,
	...
	fileModelDead = "objects/Vehicles/vespa/vespa_wreck.cgf",
--	fileModelPieces = "objects/Vehicles/vespa/vespa_pieces.cgf",	
--	fileGunModel = "Objects/Vehicles/Mounted_gun/m2.cga",
--	fileGunModel = "Objects/Vehicles/Mounted_gun/mounted_gun_vespa.cga",
...
--	CurrentPosition = nil,
	...
	szNormalModel="objects/Vehicles/vespa/vespa.cgf", 

	CarDef = {		
		--file = "objects/Vehicles/vespa/vespa.cgf", 		
		
		engine_power = 100000, -- default 110000
		engine_power_back = 100000, -- default 85000
		engine_maxrpm = 1600, -- default 700
		axle_friction = 250, -- default 650
		max_steer = 11, -- default 20
		stabilizer = 0.0,
		...
		engine_minRPM = 5,
		engine_idleRPM = 40,
		engine_shiftupRPM = 800,
		engine_shiftdownRPM = 250,
		clutch_speed = 2,
	  ...
	  cam_snap_vel = 0.01,
		...
		steer_speed_scale = 0.8,--steering speed scale at top speed
		steer_speed_scale_min = 1.5,--steering speed scale at min speed
		--steer_speed_valScale = 1.0, 
		
		max_steer_v0 = 20.0, --max steer angle
		--max_steer_kv = 0.0,
		
		steer_relaxation_v0 = 600,--(degree/s) steer speed when return to forward direction.
		--steer_relaxation_kv = 8,--15,
		-----------------------------------------------
...
--		hull0 = { mass=1859,flags=0,zoffset=.41	}, -- default mass 1859
		...
		hull1 = { mass=2500,flags=0,zoffset=-0.35,yoffset=-0.35	}, --mass:1000 -- default mass 2359
		hull2 = { mass=0,flags=1	},
		hull3 = { mass=0,flags=1	},
		
		wheel1 = { driving=1,axle=0,can_brake=1,len_max=0.23,stiffness=0,damping=-0.3, surface_id = 69, min_friction=1.0, max_friction=1.5 },
		wheel2 = { driving=1,axle=0,can_brake=1,len_max=0.23,stiffness=0,damping=-0.3, surface_id = 69, min_friction=1.0, max_friction=1.5 },
		wheel3 = { driving=1,axle=1,can_brake=1,len_max=0.23,stiffness=0,damping=-0.3, surface_id = 69, min_friction=1.0, max_friction=1.8 },
		wheel_num = 3,
	},
...
--	3 - passenger
	driverT = {
		...
		out_ang = -90,
		message = "Press USE KEY to Drive Vespa",
	...
	passengersTT = {
		{
			...
			out_ang = 90,
			message = "Press USE KEY to Jump on Vespa",
			-- invehicle animations 
			animations = {
...
VC.CreateVehicle(vespa);

--////////////////////////////////////////////////////////////////////////////////////////
function vespa:OnReset()
...
--////////////////////////////////////////////////////////////////////////////////////////
function vespa:InitPhis()
	VC.InitGroundVehiclePhysics(self,self.szNormalModel);
end	

--////////////////////////////////////////////////////////////////////////////////////////
function vespa:InitClient()
	VC.InitSeats(self, vespa);
...
--////////////////////////////////////////////////////////////////////////////////////////
function vespa:OnContactServer( player )
...
--////////////////////////////////////////////////////////////////////////////////////////
function vespa:OnContactClient( player )
...
--
--
function vespa:UpdateServer(dt)
...
--
--
function vespa:OnEventServer( id, params)
...
--
--
function vespa:AddDriver( puppet )
...
--
--
function vespa:AddGunner( puppet )
	return 0;
end
...
--
--
function vespa:AddPassenger( puppet )
...
--
--	to test-call reinf
function vespa:Event_Reinforcment( params )
...
----------------------------------------------------------------------------------------------------------------------------
--
function vespa:Event_GoPath( params )
	if(VC.FreeToUse( self )==0) then return end	-- can't use it - player is in
...
----------------------------------------------------------------------------------------------------------------------------
--
function vespa:Event_GoPatrol( params )
	if(VC.FreeToUse( self )==0) then return end	-- can't use it - player is in
...
----------------------------------------------------------------------------------------------------------------------------
--
function vespa:Event_GoChase( params )
	if(VC.FreeToUse( self )==0) then return end	-- can't use it - player is in
...
--
-- 
function vespa:Event_KillTriger( params )
...
-----------------------------------------------------------------------------------------------------
function vespa:Event_LoadPeople( params )
...
--
--
function vespa:Event_EveryoneOut()
...
--
--
function vespa:Event_Activate( params )
...
----------------------------------------------------------------------------------------------------------------------------
--
function vespa:Event_DriverIn( params )
...
function vespa:OnLoadOverall(stm)
