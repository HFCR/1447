-- paraglider 2.0 client/server fixed.
Paraglider = {
...
function Paraglider:OnInitClient()
	...
	self.szNormalModel = self.Properties.fileName;
	...
	Alive = {
		...
		OnCollide = function(self,hit)
			if (hit.fSpeed > .1) then
				if (self.driverT.entity) and (_time-self.driverT.entertime > 1) then
					self.driverT.entity.cnt.use_pressed = 1;
					VC.EveryoneOutForce(self);
				end
			end
		end,
...
function Paraglider:OnContactServer( collider )
	if (self.driverT.entity) then
		...
		if (not self.driverT.adjustme) then
			self.driverT.adjustme = 1;
	...
	VC.OnContactServerT(self,collider);
end
