--Inflatableboat.lua CIS edition v1.2 - Gamestyler (Garbitos and Mixer)
...
--////////////////////////////////////////////////////////////////////////////////////////
-- Begin EXTENDED Custom Variables
function InflatableBoat:SetSSMvars()
	if (getglobal("gr_dingySpeed")) then
		local dingySpeed=toNumberOrZero(getglobal("gr_dingySpeed"));
		self.boat_params.Speedv=dingySpeed*5000;
	end
	if (getglobal("gr_dingyTurn")) then
		local dingyTurn=toNumberOrZero(getglobal("gr_dingyTurn"));
		self.boat_params.Turn=dingyTurn*400;
	end
	if (getglobal("gr_dingyTurnDamper")) then
		local dingyTurnDamper=toNumberOrZero(getglobal("gr_dingyTurnDamper"));
		self.boat_params.Damprot=dingyTurnDamper*1000;
	end
	if (getglobal("gr_dingyTilt")) then
		local dingyTilt=toNumberOrZero(getglobal("gr_dingyTilt"));
		self.boat_params.TiltSpd=dingyTilt*80;
	end
	if (getglobal("gr_dingyMass")) then
		local dingyMass=toNumberOrZero(getglobal("gr_dingyMass"));
		self.boat_params.fMass=dingyMass*400;
	end
	if (getglobal("gr_dingyGravity")) then
		local dingyGravity=toNumberOrZero(getglobal("gr_dingyGravity"));
		self.boat_params.gravity=dingyGravity*(-9.81);
	end
	if (getglobal("gr_dingyCollideDamage")) then
		local dingyCollideDamage=toNumberOrZero(getglobal("gr_dingyCollideDamage"));
		self.fOnCollideDamage=dingyCollideDamage*0.25;
		self.fOnCollideGroundDamage=dingyCollideDamage*0.25;
		self.fOnCollideVehicleDamage=dingyCollideDamage*5;
	end
	if (getglobal("gr_dingyPushPower")) then
		local dingyPushPower=toNumberOrZero(getglobal("gr_dingyPushPower"));
		self.pushpower=dingyPushPower*500;
	end
	if (getglobal("gr_realVehixDamage")) and (getglobal("gr_dingyBulletDamage")) then
		local realVehixDamage=toNumberOrZero(getglobal("gr_realVehixDamage"));
		local dingyBulletDamage=toNumberOrZero(getglobal("gr_dingyBulletDamage"));
		if (realVehixDamage==1) then
			self.DamageParams.dmgBulletMP = nil;
			self.DamageParams.fDmgScaleBullet = dingyBulletDamage*0.2;
		else
			self.DamageParams.dmgBulletMP = dingyBulletDamage*1.0;
		end
	end
	if (getglobal("gr_vehixScaleExplosion")) then
		local vehixScaleExplosion=toNumberOrZero(getglobal("gr_vehixScaleExplosion"));
		self.Properties.ExplosionParams.ndamage = vehixScaleExplosion*100;
		self.Properties.ExplosionParams.fRadiusMax = vehixScaleExplosion*7;
		self.Properties.ExplosionParams.fRadius = vehixScaleExplosion*7;
		self.Properties.ExplosionParams.fImpulsivePressure = vehixScaleExplosion*200;
	end
end
--- End EXTENDED Custom Variables
...
function InflatableBoat:Client_OnRemoteEffect(toktable, pos, normal, userbyte)
	if (Game:IsServer()==nil) then
		GameStyler:ApplyBoatStyle(self,tonumber(pos.x),self.myName);
	end
end
...
InflatableBoat.Server = {
	...
	Alive = {
		OnBeginState = function( self )	
			VC.InitBoatCommon(self);
			if (getglobal("gr_boatstyle")~=nil) then
				self.currboatstyle = "0";
			else
				self.currboatstyle = nil;
			end
		end,
		OnContact = function(self,player)
		...
		OnUpdate = function(self,dt)
			if (self.currboatstyle) then
				if (self.currboatstyle ~= getglobal("gr_boatstyle")) then
					self.currboatstyle = getglobal("gr_boatstyle");
					GameStyler:ApplyBoatStyle(self,tonumber(self.currboatstyle),self.myName);
					--Hud:AddMessage("Boat Style".." "..self:GetName().." "..self.type.." "..self.currboatstyle);
					Server:BroadcastCommand("FX", {x=self.currboatstyle,y=0,z=0}, {x=0,y=0,z=0},self.id,4);
				end
			end
	...
	Dead = {
		OnBeginState = function( self )
			...
			if (self.currboatstyle) and (tonumber(self.currboatstyle) > 0) then
				GameStyler:ApplyBoatStyle(self,0,self.myName);
...
-- to test
function InflatableBoat:Event_DisablePhysics(params)
	self:EnablePhysics(0);
end

function InflatableBoat:Event_EnablePhysics(params)
