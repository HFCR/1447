...
--	ammoRL = 30,
	...
	DamageParams = {
		...
		dmgBulletMP = 5.0,--if this value exist in multiplayer will be used this damage for every bullet , 
	...
	fileModelDead = "objects/Vehicles/quad/quad_pieces.cgf",
	fileModelPieces = "objects/Vehicles/quad/quad_pieces.cgf",	
...
--	CurrentPosition = nil,
	...
	szNormalModel="objects/Vehicles/quad/quad.cgf", 

	CarDef = {		
		--file = "objects/Vehicles/quad/quad.cgf", 		
		
		engine_power = 300000, -- default 110000 200000
		engine_power_back = 300000, -- default 85000
		...
		stabilize_jump = 2100,--1500 this set how much to correct the car angles while it is in air.
...
--		hull0 = { mass=1859,flags=0,zoffset=.41	}, -- default mass 1859
		...
		hull1 = { mass=2000,flags=0,zoffset=-0.35,yoffset=-0.35	}, --mass:1000 -- default mass 2359
...
--	3 - passenger
	driverT = {
		...
		in_anim = "",
    out_anim = "",
    sit_anim = "quad_driver_sit_loop",
		anchor = AIAnchor.z_CARENTER_DRIVER,
		out_ang = -90,
		message = "Press Use to drive",
		-- invehicle animations 
		animations = {
		"quad_driver_sit", -- idle in animation
		"quad_driver_moving", -- driving firward
		"quad_driver_forward_hit", -- impact / break
		"quad_driver_leftturn", -- turning left
		"quad_driver_rightturn", -- turning right
		"quad_driver_reverse", -- reversing
		"quad_driver_reverse_hit", -- reversing impact / break
		},
	},
  ...
  passengersTT = {
		{
			...
			in_anim = "quad_passenger_in",
			out_anim = "quad_passenger_out",
			sit_anim = "quad_passenger_sit",
			anchor = AIAnchor.z_CARENTER_PASSENGER1,
			out_ang = 90,
			message = "Press Use to enter",
			-- invehicle animations 
			animations = {
				"quad_passenger_sit",		-- idle in animation
				"quad_passenger_moving",	-- driving firward
				"quad_passenger_forward_hit",	-- impact / break
				"quad_passenger_leftturn",	-- turning left
				"quad_passenger_rightturn",	-- turning right
				"quad_passenger_reverse",	-- reversing
				"quad_passenger_reverse_hit",	-- reversing impact / break
			},
		},
	},
...
VC.CreateVehicle(quad);

--////////////////////////////////////////////////////////////////////////////////////////
function quad:OnReset()
...
--////////////////////////////////////////////////////////////////////////////////////////
function quad:InitPhis()
	VC.InitGroundVehiclePhysics(self,self.szNormalModel);
end	

--////////////////////////////////////////////////////////////////////////////////////////
function quad:InitClient()
	VC.InitSeats(self, quad);
...
--	};
	...
	self.engine_start = Sound:Load3DSound("sounds\\vehicle\\quad\\start.wav",0,255,13,300);
	self.engine_off = Sound:Load3DSound("sounds\\vehicle\\quad\\off.wav",0,255,13,300);
	self.sliding_sound = Sound:Load3DSound("sounds\\vehicle\\break2.wav",0,150,13,300);

	-------------------------------------------------------------------------------------


	self.clutch_sound = Sound:Load3DSound("sounds\\vehicle\\quad\\gearchange.wav",0,200,13,300);
	self.land_sound = Sound:Load3DSound("sounds\\vehicle\\quad\\carclunk.wav",0,255,13,300);

	
	self.idleengine = Sound:Load3DSound("sounds\\vehicle\\quad\\idle2.wav",0,255,13,300);
...
--////////////////////////////////////////////////////////////////////////////////////////
function quad:OnContactServer( player )
...
--////////////////////////////////////////////////////////////////////////////////////////
function quad:OnContactClient( player )
...
--
--
function quad:OnEventServer( id, params)
...
--
--
function quad:AddDriver( puppet )
...
--
--
function quad:AddGunner( puppet )
	return 0;
end
...
--
--
function quad:AddPassenger( puppet )
...
--
--	to test-call reinf
function quad:Event_Reinforcment( params )
...
----------------------------------------------------------------------------------------------------------------------------
--
function quad:Event_GoPath( params )
	if(VC.FreeToUse( self )==0) then return end	-- can't use it - player is in
...
----------------------------------------------------------------------------------------------------------------------------
--
function quad:Event_GoPatrol( params )
	if(VC.FreeToUse( self )==0) then return end	-- can't use it - player is in
...
----------------------------------------------------------------------------------------------------------------------------
--
function quad:Event_GoChase( params )
	if(VC.FreeToUse( self )==0) then return end	-- can't use it - player is in
...
--
-- 
function quad:Event_KillTriger( params )
...
-----------------------------------------------------------------------------------------------------
function quad:Event_LoadPeople( params )
...
--
--
function quad:Event_EveryoneOut()
...
--
--
function quad:Event_Activate( params )
...
----------------------------------------------------------------------------------------------------------------------------
--
function quad:Event_DriverIn( params )
...
function quad:MakeAlerted()
end

--------------------------------------------------------------------------------------------------------
-- 
--function quad:InitWeapon()
...
function quad:OnSaveOverall(stm)
...
function quad:OnLoadOverall(stm)
...
function quad:CreateParticles()
	...
	local exhaust=  {
				...
				size_speed = 0.2,
				lifetime = 0.7,
				tid = System:LoadTexture("Objects/Vehicles/quad/exhaust_particle.dds"),
	...
	local pos_eng = self:GetHelperPos("exhaust_pos");	
	...
	local speed = self.cnt:GetVehicleVelocity();
   	...
   	if ( speed < 10 ) then	       
        Particle:CreateParticle(pos_eng,dir_eng,exhaust);
	end
