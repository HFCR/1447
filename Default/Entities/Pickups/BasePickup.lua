...
function BasePickup:AttemptPick(collider, entering)
	if (collider.type == "Player") and (collider.cnt.health>0) then
		if (self.weapon) then
			if (GameRules.insta_play) or (collider.dont_pick_the_gun) then
				return
			end
			if (Game:IsMultiplayer()) and (strfind(self.weapon,"RL") ~= nil) and (toNumberOrZero(getglobal("gr_norl"))==1) then
				return
			end
		end
		if (collider.theVehicle) then return end
		if (GameRules:IsInteractionPossible(collider,self)==nil) then return end
		if (collider.su_turret) then return end
		if (self.not_for_ai) and (collider.POTSHOTS) then return end
		...
		if (picked) or (self.ammopicked) then
			...
			if (self.Event_KeyCardPickUp) and (self.Properties.KeyNumber) and (picked) then
				local slot=Server:GetServerSlotByEntityId(collider.id);
				if (slot) then
					slot:SendCommand("GI K "..self.Properties.KeyNumber);
				end
			elseif (self.Event_PickupExploPickUp) and (collider.explosives) then
				local slot=Server:GetServerSlotByEntityId(collider.id);
				if (slot) then
					local itemCount = 0;
					...
					slot:SendCommand("GI E "..itemCount);
				end
			end
...
BasePickup.Server={
	...
	Active={
		...
		OnTimer = function(self)
			if (not self.stay_for_coop) then
	...
	Dropped={
		...
		OnTimer = function(self)
			------
			if (GameRules.cooppicks) then -- also protect regular 'on-map' pickups
				self.stay_for_coop = 1;
				self.deleteOnGameReset = 1;
				local cpcount = getn(GameRules.cooppicks);
				if (cpcount < 10) then
					tinsert(GameRules.cooppicks,floor(self.id));
				else
					for i, cid in GameRules.cooppicks do
						if (i ~= "n") then
							local j_ent = System:GetEntity(cid);
							if (j_ent) and (j_ent.stay_for_coop) then
								j_ent.stay_for_coop = nil;
								j_ent:SetTimer(3000);
							end
							tremove(GameRules.cooppicks,i);
							break;
						end
					end
					tinsert(GameRules.cooppicks,floor(self.id));
	...
	Inactive={
		----------- generic pickup spawns once at game round in coop game
		OnBeginState = function(self)
			if (self.Properties.RespawnTime<=0) then
				self:EnableSave(nil);
			end
			if (self.Properties.RespawnTime>0) then
				if (self.classname == "PickupGeneric") and (GameRules.cooppicks) then
				else
					self:SetTimer(self.Properties.RespawnTime*1000);
				end
			end;
		end,
		----------------------------------
...
BasePickup.Client={
	OnInit = function(self)
		...
		-- Mixer: item dropping improvement (MOVED HERE)
		if (self:GetName()=="") then
			if (self.weapon) and (self.ammo_type) then
			self:SetAngles({x=90,z=random(0,359)}); end
		end
	end,
	Active={
		-------
		OnBeginState = function(self)
				...
				self.zup=random(0,1);
				if (self.cis_respsnd) then
					local ppos = self:GetPos();
					Particle:SpawnEffect(ppos,g_Vectors.v001,'misc.sparks.a',3);
					Sound:SetSoundPosition(self.cis_respsnd,ppos);
					Sound:PlaySound(self.cis_respsnd);
				end
		end,
		OnContact = function(self, collider)
			...
			if (self.weapon and ws) then
				...
				if (has_hands) and (count==4) and (not collider.cnt.lock_weapon) then
					count=3;
				end
				if (collider.cnt.weapon) then
				if((count==4 or has_weapon) and (has_weapon~=1) and not collider.cnt.lock_weapon) then
				...
				if (collider.cnt.weapon.name == "Hands" or collider.cnt.weapon.name == "EngineerTool") and (has_weapon == nil) then
					self.sound_played=1;
				end
				end
			end
	...
	Picked={
		OnBeginState = function(self)
			...
			if (self.Properties.RespawnTime > 0) then
				if (not self.cis_respsnd) then
					self.cis_respsnd = Sound:Load3DSound("Sounds/items/cis_item_respawn.wav",SOUND_UNSCALABLE,250,5,32);
				end
			else
				self.cis_respsnd = nil;
			end
		end,
...
function __PickWeapon(self,collider, entering)
	...
	collider.lastfodpkuptakeaway = "";
	for i,val in ws do
		if (val~=0) then
			if WeaponClassesEx[val.name].food then
				collider.lastfodpkuptakeaway = ""..val.name;
			else
				count=count+1;
			end
			if (val.name=="Hands") or (val.name=="EngineerTool") then
				has_hands=1;
				if (val.name~="Hands") and (collider.items) then
					-- plus code to handle wrenchtool in inventory correctly!
					collider.items.wrenchtool = 1;
				end
			elseif (val.name==self.weapon) then has_weapon=1; break; end
		end 
	end

if (has_hands) then
if (collider.cnt.weapon) and (collider.cnt.weapon.name ~= "Hands" and collider.cnt.weapon.name ~= "EngineerTool") then
	has_hands = nil;
end
if (count == 4) and (not has_weapon) then
if (has_hands) then collider.cnt:SetCurrWeapon(); end
collider.cnt:MakeWeaponAvailable(9,0);
collider.cnt:MakeWeaponAvailable(27,0);
count = 3; end
end
	if (count == 4) then
	self.hadFullSlots = 1;
	...
	if (count<4 and (not has_weapon)) then
		if (collider.cnt.MakeWeaponAvailable) then
			if collider.lastfodpkuptakeaway ~= "" then
				if collider.cnt.weapon and collider.cnt.weapon.name == collider.lastfodpkuptakeaway then
					has_hands = 1;
				end
				collider.cnt:MakeWeaponAvailable(WeaponClassesEx[collider.lastfodpkuptakeaway].id,0);
				collider.cnt:MakeWeaponAvailable(classid);
				collider.cnt:MakeWeaponAvailable(WeaponClassesEx[collider.lastfodpkuptakeaway].id);
			else
				collider.cnt:MakeWeaponAvailable(classid);
			end
			if collider.cnt.weapon == nil and has_hands == nil then
				collider.cnt:SelectFirstWeapon();
			else
				if (has_hands) or (tonumber(p_weapon_switch) == 1 or (self.hadFullSlots == 1 and count == 3)) then
	...
	if (self.Properties.RespawnTime>0) and (self.ammopicked) and (self.ammopicked==1) and (not wpicked) then
		collider.dont_pick_the_gun = 1;
		local touchedgun = {
			classid = Game:GetEntityClassIDByClassName("Pickup"..self.weapon),
			pos = new(self:GetPos()),
			angles = new(self:GetAngles()),
			name = self:GetName().."_tou4d",
		}
		touchedgun.pos.z = touchedgun.pos.z + 0.2;
		touchedgun = Server:SpawnEntity(touchedgun);
		if (touchedgun) then
			touchedgun.autodelete=1;
			touchedgun:EnableSave(1);
			touchedgun:SetFadeTime(self.Properties.RespawnTime*1);
			touchedgun.Properties.Amount = self.Properties.Amount*1;
			touchedgun.Properties.Amount2 = self.Properties.Amount2*1;
			touchedgun:GotoState("Active");
		end
		collider.dont_pick_the_gun = nil;
	end
