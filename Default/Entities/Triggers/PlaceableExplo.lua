...
function PlaceableExplo:PrintMessage(id,msg)
	if (id) and (Game:IsServer()) then
		if (id==0) and (_localplayer) then
			id=_localplayer.id;
		end
	...
	for i,val in player.explosives do
		if (val==1) then
			if (self.Properties.bAutomaticPlaceable ~= 1) then
				...
				elseif (not player.cnt.use_pressed) then
					if (_localplayer) and (_localplayer == player) then
			...
			local slot=Server:GetServerSlotByEntityId(player.id);
			if (slot) then
				local itemCount = 0;
				for i,val in player.explosives do
					if (val==1) then
...
function PlaceableExplo:OnRemoteEffect(toktable, pos, normal, userbyte)
