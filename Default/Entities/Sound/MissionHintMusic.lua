...
function MissionHintMusic:OnSave(stm)
	stm:WriteInt(self.HintCount);
end

function MissionHintMusic:OnLoad(stm)
	self:OnReset();
	self.HintCount = stm:ReadInt();
end

function MissionHintMusic:OnPropertyChange()
	...
	if (self.sound ~= nil) then
		...
		Sound:SetSoundVolume(self.sound,self.Properties.iVolume*getglobal("s_MusicVolume"));		
	end;
end

function MissionHintMusic:OnReset()
	self:NetPresent(nil);
	self.SkipCount = 0;
	self.HintCount = 1;
	self.skipped = 0;
...
function MissionHintMusic:Play()
...
function MissionHintMusic:StopSound()
...
function MissionHintMusic:LoadSnd()
...
function MissionHintMusic:OnRemoteEffect(toktable, pos, normal, userbyte)
...
function MissionHintMusic:Event_Play(sender)
...
function MissionHintMusic:Event_Stop( sender )
	if (Game:IsServer()) then
...
function MissionHintMusic:Event_Enable( sender )
	self.Properties.bEnabled = 1;
end

function MissionHintMusic:Event_Disable( sender )
	self.Properties.bEnabled = 0;
end

function MissionHintMusic:OnWrite( stm )
end

function MissionHintMusic:OnRead( stm )
end