-- improved weapon scope
WeaponScope={
	overlay_func=nil,
	v_01010={x=0.02,y=0.04,z=0.02},
	temp={},
	target_pos_temp={},
	LastZoom=1,
}
...
function WeaponScope:OnUpdate()
	...
	if (w.DoesFTBSniping) then
		...
		if (_time-self.fade<0.3) then
			self.state=1;
			ZoomView:Reset();
			if (w.ScopeToScreen) then
				w:ScopeToScreen((_time-self.fade)/0.3);
			end
		else
	...
	if(w.AimMode)then
		if (self.target_pos == nil) then
			...
			if (_localplayer.fireparams.aim_offset) then
				pos.x=_localplayer.fireparams.aim_offset.x;
				pos.y=_localplayer.fireparams.aim_offset.y;
				pos.z=_localplayer.fireparams.aim_offset.z;
			else
