...
function DefaultZoomHUD:DrawHUD(ZoomStep)
	if ( ZoomStep ~= self.PrevZoomStep ) then
		if (_localplayer.cnt.weapon.name=="OICW") then
			_localplayer.makezoomstep = 0.523;
		elseif (_localplayer.cnt.weapon.name=="AG36") then
			local ZoomTxtDetail = {0.55,0.392};
	...
	if (DefaultZoomHUD.MulMask) then
		...
		if(_localplayer.cnt.weapon.name=="OICW") then
		  ...
		  --System:Draw2DLine(400.5, 30, 400.5, 80,0,0,0,1);
			...
			System:Draw2DLine(400-92+Hud.hdadjust*0.5, 300, 400-2, 300,0,0,0,1);
			System:Draw2DLine(400+2, 300, 400+92-Hud.hdadjust*0.5, 300,0,0,0,1);

			System:Draw2DLine(400-25, 380, 400+25, 380,0,0,0,1);
			System:Draw2DLine(400-15, 420, 400+15, 420,0,0,0,1);

			posZoomX=220+Hud.hdadjust*0.48;
			posZoomY=293;
			posDistX=502-Hud.hdadjust*0.48;
			posDistY=293;
		end
...
-- mixer: widescreen support
		System:DrawImageColorCoords( DefaultZoomHUD.MulMask, Hud.hdadjust*1.52, 0, 400*Hud.hd_sc_mult, 600   , 7, 1, 1, 1, 1, fAdjustTexW, 1-fAdjustTexH, 1-fAdjustTexW, fAdjustTexH);
		System:DrawImageColorCoords( DefaultZoomHUD.MulMask, 800-Hud.hdadjust*1.52, 0, -400*Hud.hd_sc_mult, 600, 7, 1, 1, 1, 1, fAdjustTexW, 1-fAdjustTexH, 1-fAdjustTexW, fAdjustTexH);
		if (Hud.hdadjust > 0) then
		System:DrawImageColorCoords( DefaultZoomHUD.MulMask, Hud.hdadjust*1.52, 0, -400*Hud.hd_sc_mult, 600   , 7, 1, 1, 1, 1, fAdjustTexW, 1-fAdjustTexH, 1-fAdjustTexW, fAdjustTexH);
		System:DrawImageColorCoords( DefaultZoomHUD.MulMask, Hud.hdadjust*1.52+400*Hud.hd_sc_mult*2, 0, 400*Hud.hd_sc_mult,600,7,1,1,1,1,fAdjustTexW, 1-fAdjustTexH, 1-fAdjustTexW, fAdjustTexH);
		end
	end
