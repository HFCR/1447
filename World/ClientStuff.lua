...
function ClientStuff:UpdateScoreboard()
	...
	for iY=0,iLines-1 do
		...
		if (idThisClient) and (idThisClient~=0) then
			...
			local playerScore={
				[1]= { iVal=iSuicides, iSort=1 }, -- effiency score, sort by bigger
				[2]= { iVal=iScore, iSort=1 }, -- kills score, sort by bigger
				[3]= { iVal=iDeaths, iSort=0 }, -- deaths score, sort by smaller
...
ClientStuff.ServerCommandTable["MMD"]=function(String,toktable)
	if (toktable[2]) then
		...
		elseif (toktable[2]=="refuse") then
			if (UI) then
				UI.MessageBox("@Error", "@world_profile_refuse "..getglobal("p_name"));
			end
		end
	end
end;
