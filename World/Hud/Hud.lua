...
function Hud:OnInit()
	...
	self.moneysign = System:LoadImage("textures/hud/moneysign_world_hud");
	Hud.prv_money_amount = 0;
...
function Hud:OnUpdate()
	...
	if (player) then
		if (player.entity_type~="spectator") and (player.cnt ~= nil) then
				...
				----------------
				if Hud.cl_money_amount then
					%System:DrawImageColor(Hud.moneysign, 358, 576, 90, 19, 4, 1, 1, 1, 0.8);
					local nsx,nsy = %Game:GetHudStringSize(Hud.cl_money_amount, 16, 16);
					nsx = 400-nsx*0.5;
					Game:SetHUDFont("radiosta", "binozoom");
					%Game:WriteHudString(nsx,576,Hud.cl_money_amount, 0, 1, 1, 1, 16, 16);
					if Hud.prv_money_amount ~= Hud.cl_money_amount then
						Hud.prv_money_amount = floor(Hud.cl_money_amount - Hud.prv_money_amount);
						if Hud.prv_money_amount > 0 then
							Hud:AddMessage("$1+"..Hud.prv_money_amount);
						else
							Hud:AddMessage("$1"..Hud.prv_money_amount);
						end
					end
					Hud.prv_money_amount = Hud.cl_money_amount;
				end
				----------------------
...
if (player.cnt) and (player.cnt.GetViewIntersection) then
	...
	if obj and obj.ent then
		...
		elseif (obj.ent.classname == "BuyObject") then
			if (obj.len <= 1.4) then
				--Hud.label = "@World_Buy ("..obj.ent.Properties.nAmount..") "..obj.ent.Properties.nPrice;
				%Game:SetHUDFont("hud", "ammo");
				if obj.ent.Properties.nPrice == 0 and obj.ent.Properties.nAmount == 0 then
					Hud.cis_totalsec = "@WORLD_GetSomeJob";
				elseif obj.ent.Properties.sBuyCommand == "House" then
					Hud.cis_totalsec = "@RentHouse "..obj.ent.Properties.nPrice.." F";
				elseif obj.ent.Properties.nAmount > 1 then
					Hud.cis_totalsec = "@World_Buy "..obj.ent.Properties.nAmount.." @"..obj.ent.Properties.sBuyCommand.." ("..obj.ent.Properties.nPrice.." F)";
				else
					Hud.cis_totalsec = "@World_Buy @"..obj.ent.Properties.sBuyCommand.." ("..obj.ent.Properties.nPrice.." F)";
				end
				local strszex,strszey = Game:GetHudStringSize(Hud.cis_totalsec, 21, 21);
				%System:DrawImageColor(Hud.white_dot, 398-(strszex/2), 319, strszex+4, 23, 4,0,0,0.2,0.5);
				Game:WriteHudString( 400-(strszex/2), 320, Hud.cis_totalsec, 1, 1, 1, 0.86, 21, 21);
				if (player.cnt.use_pressed) then
					player.cnt.use_pressed = nil;
					Client:SendCommand("BUY "..obj.ent.id);
				end
			end
		end
	end
